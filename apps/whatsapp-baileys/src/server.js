...
function authorized(req) {
  if (!SERVICE_TOKEN) return process.env.NODE_ENV !== 'production';
  return req.header('x-whatsapp-service-token') === SERVICE_TOKEN;
}
...
function safeAccountId(value) {
  ...
  return /^[a-fA-F0-9-]{12,64}$/.test(id) ? id : null;
}
...
async function notifyBackend(payload) {
  if (!BACKEND_WEBHOOK_URL || !BACKEND_WEBHOOK_TOKEN) {
    logger.error('WhatsApp backend webhook is not fully configured');
    return;
  }
  try {
    const response = await fetch(BACKEND_WEBHOOK_URL, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-whatsapp-webhook-token': BACKEND_WEBHOOK_TOKEN },
      body: JSON.stringify(payload),
    });
    if (!response.ok) logger.warn({ status: response.status }, 'WhatsApp webhook rejected');
  } catch (error) { logger.warn({ err: error }, 'WhatsApp webhook delivery failed'); }
}
...
async function connectAccount(accountId, options = {}) {
  ...
  socket.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') { state.status = 'connected'; state.registered = true; state.reconnectAttempt = 0; state.qr = null; state.qrDataUrl = null; state.pairingCode = null; state.phoneNumber = socket.user?.id?.split(':')[0]?.replace(/\D/g, '') || state.phoneNumber; logger.info({ accountId }, 'WhatsApp account connected'); }
    if (connection === 'close') {
      const code = lastDisconnect?.error?.output?.statusCode;
      state.socket = null; state.status = code === DisconnectReason.loggedOut ? 'logged_out' : 'disconnected'; state.lastError = lastDisconnect?.error?.message || `connection closed (${code ?? 'unknown'})`;
      if (code !== DisconnectReason.loggedOut) { const delay = Math.min(30000, 1000 * (2 ** Math.min(state.reconnectAttempt, 5))); state.reconnectAttempt += 1; clearTimeout(state.reconnectTimer); state.reconnectTimer = setTimeout(() => connectAccount(accountId).catch((error) => logger.warn({ err: error, accountId }, 'WhatsApp reconnect failed')), delay); }
    }
    accounts.set(accountId, state);
  });
  ...
  socket.ev.on('messages.upsert', async ({ messages, type }) => {
    for (const message of messages) {
      ...
      const mediaKind = message.message.imageMessage ? 'image' : message.message.videoMessage ? 'video' : message.message.documentMessage ? 'document' : message.message.audioMessage ? 'audio' : 'text';
      ...
      await notifyBackend({ event: 'message.received', accountId, type, messageId: message.key?.id, from: jid, pushName: message.pushName || null, kind: mediaKind === 'text' ? 'text' : 'media', text, timestamp: message.messageTimestamp ? Number(message.messageTimestamp) : Date.now(), raw: { key: message.key, message: message.message } });
    }
  });
  ...
  socket.ev.on('messages.update', async (updates) => {
    for (const item of updates) {
      ...
      if (!id || item?.update?.status == null) continue;
      const status = Number(item.update.status);
      const mapped = status >= 4 ? 'read' : status >= 3 ? 'delivered' : status === 0 ? 'failed' : 'sent';
      await notifyBackend({ event: 'message.status', accountId, messageId: id, status: mapped, raw: item });
    }
  });
  return state;
}

async function restorePersistedAccounts() {
  await ensureDir(SESSION_ROOT);
  const entries = await fs.readdir(SESSION_ROOT, { withFileTypes: true });
  const accountIds = entries.filter((entry) => entry.isDirectory() && safeAccountId(entry.name)).map((entry) => entry.name);
  for (const accountId of accountIds) {
    try {
      const state = await connectAccount(accountId);
      if (!state.registered) {
      ...
      logger.warn({ err: error, accountId }, 'Failed to restore WhatsApp session');
    }
  }
  logger.info({ restoredAccounts: accountIds.length }, 'WhatsApp sessions restored');
}
...
app.post('/v1/accounts/:accountId/connect', requireAuth, async (req, res) => { const accountId = safeAccountId(req.params.accountId); if (!accountId) return res.status(400).json({ success: false, message: 'Invalid accountId' }); const state = await connectAccount(accountId); if (req.body?.phoneNumber && !state.registered && state.socket?.requestPairingCode) { state.pairingCode = await state.socket.requestPairingCode(String(req.body.phoneNumber).replace(/\D/g, '')); state.status = 'pairing_required'; } return res.json({ success: true, data: { accountId, status: state.status, hasQr: Boolean(state.qrDataUrl), pairingCode: state.pairingCode } }); });
...
restorePersistedAccounts().catch((error) => logger.error({ err: error }, 'WhatsApp session restore failed'));
