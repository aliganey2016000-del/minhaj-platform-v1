...
function getState(accountId) {
  return accounts.get(accountId) || { socket: null, status: 'disconnected', qr: null, qrDataUrl: null, pairingCode: null, phoneNumber: null, lastError: null, reconnectTimer: null, reconnectAttempt: 0 };
}
...
async function notifyBackend(payload) {
  ...
  try {
    const response = await fetch(BACKEND_WEBHOOK_URL, {
      method: 'POST',
      headers: { 'content-type': 'application/json', ...(BACKEND_WEBHOOK_TOKEN ? { 'x-whatsapp-webhook-token': BACKEND_WEBHOOK_TOKEN } : {}) },
