...
export const status = async (_req: Request, res: Response): Promise<Response> =>
  ApiResponse.success(res, { configured: isWhatsAppConfigured(), provider: 'Meta WhatsApp Cloud API' });
...
export const send = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (scopedSchool && !mongoose.isValidObjectId(scopedSchool as string)) {
  ...
  try {
    const result = await sendWhatsAppMessage({
      ...
      templateName,
      languageCode,
      components: componentsFromBody(components),
    });
    message.status = 'sent';
    ...
    message.error = error?.response?.data?.error?.message || error?.message || 'WhatsApp send failed';
    await message.save();
...
export const history = async (req: Request, res: Response): Promise<Response> => {
  ...
  const [items, total] = await Promise.all([
    WhatsAppMessage.find(scoped)
      .populate('parent', 'parentId phone relationship')
