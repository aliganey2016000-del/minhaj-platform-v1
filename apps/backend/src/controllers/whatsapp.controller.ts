  ...
  return WhatsAppConversation.findOneAndUpdate(
    { organization: input.organizationId, phone },
    {
      $set: {
        ...
        lastMessagePreview: input.preview?.slice(0, 500),
...
export const connect = async (req: Request, res: Response): Promise<Response> => {
  ...
  return ApiResponse.success(res, await connectBaileysAccount(organizationId, phoneNumber), 'WhatsApp connection started');
};
...
export const webhook = async (req: Request, res: Response): Promise<Response> => {
  ...
  const parent = await Parent.findOne({ phone: from }).lean();
  const conversation = await upsertConversation({
    organizationId,
    phone: from,
    direction: 'inbound',
    preview: typeof payload.text === 'string' ? payload.text : '[WhatsApp message]',
    parentId: parent?._id,
    schoolId: parent?.school,
    contactName: payload.pushName ? String(payload.pushName) : undefined,
    unreadIncrement: 1,
  });
  ...
  await WhatsAppMessage.create({
    ...
    parent: parent?._id,
    direction: 'inbound',
    kind: payload.kind === 'media' ? 'media' : 'text',
...
export const send = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (conversationId) {
    conversation = await WhatsAppConversation.findOne({ _id: conversationId, organization: organizationId });
    if (!conversation) throw new NotFoundError('WhatsApp conversation');
    recipient = conversation.phone;
    if (conversation.parent) parent = await Parent.findById(conversation.parent).lean();
  }
  if (parentId) {
  ...
  if (!conversation) {
    conversation = await upsertConversation({
      ...
      phone: recipient,
      direction: 'outbound',
      preview: text || templateName,
      parentId: parent?._id,
      schoolId: scopedSchool as mongoose.Types.ObjectId | undefined,
      contactName: parent?.name || undefined,
    });
  }
  ...
  try {
    ...
    message.error = error?.response?.data?.message || error?.response?.data?.error?.message || error?.message || 'WhatsApp send failed';
    await message.save();
...
export const conversations = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (search) filter.$or = [{ phone: { $regex: search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' } }, { contactName: { $regex: search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' } }];
  const [items, total] = await Promise.all([
    WhatsAppConversation.find(filter).populate('parent', 'parentId phone relationship').populate('assignedTo', 'email').sort({ lastMessageAt: -1, updatedAt: -1 }).skip((page - 1) * limit).limit(limit).lean(),
...
export const conversationMessages = async (req: Request, res: Response): Promise<Response> => {
  const organizationId = requestOrganizationId(req);
  if (!organizationId || !mongoose.isValidObjectId(organizationId)) throw new BadRequestError('A valid organization is required');
  const conversation = await WhatsAppConversation.findOne({ _id: req.params.conversationId, organization: organizationId }).lean();
  if (!conversation) throw new NotFoundError('WhatsApp conversation');
  const page = Math.max(1, Number(req.query.page) || 1);
  const limit = Math.min(100, Math.max(1, Number(req.query.limit) || 50));
  const filter = { organization: organizationId, conversation: conversation._id };
  const [items, total] = await Promise.all([
    WhatsAppMessage.find(filter).populate('createdBy', 'email').sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).lean(),
...
export const markConversationRead = async (req: Request, res: Response): Promise<Response> => {
  const organizationId = requestOrganizationId(req);
  if (!organizationId || !mongoose.isValidObjectId(organizationId)) throw new BadRequestError('A valid organization is required');
  const conversation = await WhatsAppConversation.findOneAndUpdate({ _id: req.params.conversationId, organization: organizationId }, { $set: { unreadCount: 0 } }, { new: true }).lean();
  if (!conversation) throw new NotFoundError('WhatsApp conversation');
...
export const updateConversation = async (req: Request, res: Response): Promise<Response> => {
  const organizationId = requestOrganizationId(req);
  if (!organizationId || !mongoose.isValidObjectId(organizationId)) throw new BadRequestError('A valid organization is required');
  const status = ['open', 'closed', 'archived'].includes(String(req.body?.status)) ? String(req.body.status) : undefined;
  if (!status) throw new BadRequestError('status must be open, closed, or archived');
  const conversation = await WhatsAppConversation.findOneAndUpdate({ _id: req.params.conversationId, organization: organizationId }, { $set: { status } }, { new: true }).lean();
  if (!conversation) throw new NotFoundError('WhatsApp conversation');
  return ApiResponse.success(res, conversation);
};
...
export const history = async (req: Request, res: Response): Promise<Response> => {
  ...
  const organizationId = requestOrganizationId(req);
  if (organizationId && mongoose.isValidObjectId(organizationId)) scoped.organization = organizationId;
  const [items, total] = await Promise.all([
