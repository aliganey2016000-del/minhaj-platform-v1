...
export const getForDate = async (req: Request, res: Response): Promise<Response> => {
  ...
  const staff = await User.find(staffFilter)
    .select('_id email phone title department organizationId')
...
export const mark = async (req: Request, res: Response): Promise<Response> => {
  ...
  const record = await StaffAttendance.findOneAndUpdate(
    { organizationId: staff.organizationId, user: staff._id, date: day },
    { $set: { status, notes: notes || '', markedBy: req.user!.userId, markedAt: new Date() } },
...
export const history = async (req: Request, res: Response): Promise<Response> => {
  ...
  const records = await StaffAttendance.find(filter)
    .populate({ path: 'user', select: 'email phone title', populate: { path: 'profile', select: 'firstName lastName' } })
    .sort({ date: -1 })
    .limit(200)
    .lean();
