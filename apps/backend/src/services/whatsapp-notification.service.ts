...
export async function queueWhatsAppNotification(input: QueueWhatsAppNotificationInput) {
  ...
  if (input.parentId) {
    const preference = await WhatsAppNotificationPreference.findOne({ organizationId, parentId: asObjectId(input.parentId), category: input.category }).lean();
  ...
  const job = await WhatsAppNotificationJob.findOneAndUpdate(
    ...
    { $setOnInsert: { organizationId, parentId: input.parentId ? asObjectId(input.parentId) : undefined, recipientPhone: normalizePhone(phone), category: input.category, eventType: input.eventType, payload: input.payload, maxAttempts: input.maxAttempts ?? 5, status: 'queued', nextAttemptAt: new Date(), idempotencyKey: input.idempotencyKey } },
...
export async function processWhatsAppNotificationJob(jobId: string) {
  ...
  try {
    const result = await sendWhatsAppMessage({ to: job.recipientPhone, text: renderText(job.payload), organizationId: job.organizationId.toString() });
    await WhatsAppNotificationJob.updateOne({ _id: job._id }, { $set: { status: 'sent', providerMessageId: result.providerMessageId, sentAt: new Date(), lastError: null } });
    ...
    await WhatsAppNotificationJob.updateOne({ _id: job._id }, { $set: { status: exhausted ? 'failed' : 'queued', nextAttemptAt: new Date(Date.now() + delayMs), lastError: message } });
