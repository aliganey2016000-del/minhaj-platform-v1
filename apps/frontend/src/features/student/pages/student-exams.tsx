...
export function StudentExams() {
  ...
  const nextExam = [...withState].filter((x) => (x.state === 'upcoming' || x.state === 'active') && getStart(x.exam)).sort((a, b) => getStart(a.exam)! - getStart(b.exam)!)[0];
  ...
  const renderExam = ({ exam: e, state }: { exam: Exam; state: ExamState }) => {
    ...
    const dateLabel = start
      ...
      : '';
    const roomSeat = e.mySeatRoom
      ? `${e.mySeatRoom}${e.mySeatBuilding ? ` · ${e.mySeatBuilding}` : ''}${e.mySeat ? ` · Seat ${e.mySeat}` : ''}`
      : e.room;
    const footerVisible = state === 'active' || state === 'completed' || state === 'missed' || (state === 'upcoming' && !!start);
    return (
      <article key={e._id} className={`group relative overflow-hidden rounded-2xl border bg-[var(--color-surface-primary)] transition-all hover:-translate-y-0.5 hover:shadow-xl ${state === 'active' ? 'border-emerald-400/60 shadow-emerald-500/10' : 'border-[var(--color-border-default)]'}`}>
        <div className="flex gap-3 p-4 sm:p-5">
          <div className={`hidden h-16 w-1 shrink-0 rounded-full sm:block ${meta.dot}`} />
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-lg font-bold leading-snug text-[var(--color-text-primary)] sm:text-xl">{examTitle(e, lang)}</h3>
            <p className="mt-0.5 truncate text-sm font-medium text-[var(--color-text-secondary)]">{toTitleCase(e.title)} <span className="text-[var(--color-text-tertiary)]">· {getCat(e.course?.category || '')}</span></p>
            {dateLabel && <p className="mt-2 text-sm font-bold text-[var(--color-text-primary)]">📅 {dateLabel}</p>}
            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-[var(--color-text-tertiary)]">
              <span>🕐 <b className="text-[var(--color-text-secondary)]">{formatTime(e)}</b></span>
              {roomSeat && <span>📍 <b className="text-[var(--color-text-secondary)]">{roomSeat}</b></span>}
        ...
        {footerVisible && (
          <div className="space-y-2 border-t border-[var(--color-border-default)] bg-[var(--color-surface-secondary)]/40 px-4 py-3 sm:px-5">
            {state === 'upcoming' && start && <Countdown target={start} lang={lang} />}
            {state === 'active' && <button onClick={() => navigate(`/student/exams/active?examId=${e._id}`)} className="w-full rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 px-4 py-3 text-sm font-bold text-white shadow-md shadow-emerald-500/20 transition hover:brightness-105 active:scale-[.98]">🚀 {lang === 'so' ? 'Bilow Imtixaanka' : 'Start Exam'}</button>}
            {state === 'completed' && <button onClick={() => navigate(`/student/exams/${e._id}/review`)} className="w-full rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] px-4 py-3 text-sm font-semibold text-[var(--color-text-primary)] hover:bg-[var(--color-surface-tertiary)]">📋 {lang === 'so' ? 'Eeg Jawaabaha' : 'Review Answers'}</button>}
            {state === 'missed' && (e.myRetakeRequestStatus === 'pending' || e.myRetakeRequestStatus === 'under_review' ? <span className="block rounded-xl bg-blue-500/10 px-3 py-2.5 text-center text-[11px] font-semibold text-blue-600 dark:text-blue-300">⏳ Retake request pending</span> : e.myRetakeRequestStatus === 'rejected' ? <span className="block rounded-xl bg-red-500/10 px-3 py-2.5 text-center text-[11px] font-semibold text-red-600 dark:text-red-300">🚫 Retake request rejected</span> : <button onClick={() => { setRetakeModal(e); setRetakeReason(''); setRetakeError(''); }} className="w-full rounded-xl border border-primary-500/30 bg-primary-500/10 px-4 py-3 text-sm font-semibold text-primary-700 dark:text-primary-300 hover:bg-primary-500/15">🔁 Request Retake</button>)}
          </div>
        )}
    ...
    <div className="min-h-full px-4 pb-12 pt-20 sm:px-6 lg:px-10 lg:pt-10">
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        {viewMode === 'other' ? (
          ...
          <>
            <div className="flex w-full gap-1.5 overflow-x-auto rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-1.5 scrollbar-none">
              {statusCards.map((s) => <button key={s.key} onClick={() => setTab(s.key)} className={`flex shrink-0 items-center gap-1.5 rounded-xl px-3.5 py-2 text-xs font-semibold transition-all ${tab === s.key ? 'bg-primary-600 text-white shadow-sm' : 'text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-tertiary)]'}`}><span>{s.icon}</span>{s.label}<span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${tab === s.key ? 'bg-white/20' : 'bg-[var(--color-surface-tertiary)]'}`}>{s.count}</span></button>)}
            </div>
