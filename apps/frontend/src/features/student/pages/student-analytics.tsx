    ...
    <div className="min-h-screen bg-[var(--color-surface-primary)] px-4 py-8">
      <div className="mx-auto max-w-2xl rounded-2xl border border-red-500/25 bg-[var(--color-surface-secondary)] p-6 text-center shadow-sm sm:p-8">
        ...
        <button
          type="button"
          onClick={onRetry}
          className="mt-5 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
        >
...
export function StudentAnalytics() {
  ...
  if (isAttemptDetail) {
    return <StudentAttemptDetail courseId={courseId!} activityType={activityType!} activityId={activityId!} />;
  }

  if (loading) return <PageLoader />;
  if (error && data.courses.length === 0) return <LoadError message={error} onRetry={() => void loadPerformance()} />;
        ...
        <div className="mx-auto max-w-[1200px] space-y-5 px-3 pb-12 pt-4 sm:px-6 sm:pt-5 lg:px-8">
          <header>
            <div className="flex items-start gap-3 sm:items-center">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm leading-5 text-[var(--color-text-tertiary)]">Select a course to view its lesson activity and progress chart.</p>
              </div>
            </div>
          ...
          {error && (
            <div className="flex flex-col gap-3 rounded-xl border border-amber-500/25 bg-amber-500/5 px-4 py-3 text-sm text-amber-300 sm:flex-row sm:items-center sm:justify-between">
              <span>{error}</span>
              <button type="button" onClick={() => void loadPerformance()} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-amber-500/30 px-3 text-xs font-black hover:bg-amber-500/10">
          ...
          <section>
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              ...
              {data.courses.length > 0 && <span className="rounded-full border border-[var(--color-border-default)] bg-[var(--color-surface-secondary)] px-3 py-1.5 text-xs font-bold text-[var(--color-text-secondary)]">{data.courses.length} courses</span>}
            </div>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <h3 className="break-words text-base font-black text-[var(--color-text-primary)]">{titleOf(course.title)}</h3>
                  ...
                  <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
                    <div className="col-span-2 rounded-xl bg-[var(--color-surface-primary)] p-3 sm:col-span-1 sm:bg-transparent sm:p-0">
            ...
            {data.courses.length === 0 && (
              <div className="rounded-2xl border border-dashed border-[var(--color-border-default)] bg-[var(--color-surface-secondary)] px-5 py-12 text-center">
                ...
                <p className="mt-3 text-sm font-black text-[var(--color-text-secondary)]">No course performance yet</p>
                <p className="mx-auto mt-1 max-w-md text-xs leading-5 text-[var(--color-text-tertiary)]">Your enrolled courses will appear here. Complete an interactive lesson check or quiz to begin building performance history.</p>
              </div>
            )}
          </section>
        </div>
        ...
        <section className="overflow-hidden rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-secondary)] shadow-sm">
          <div className="border-b border-[var(--color-border-default)] p-2">
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setActiveTab('activity')} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl px-2 py-2 text-xs font-bold transition sm:px-3 sm:text-sm ${activeTab === 'activity' ? 'bg-emerald-500/15 text-emerald-500' : 'text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-tertiary)]'}`}>
                <ListChecks className="h-4 w-4 shrink-0" /> <span>Lesson Activity</span>
              </button>
              <button type="button" onClick={() => setActiveTab('chart')} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl px-2 py-2 text-xs font-bold transition sm:px-3 sm:text-sm ${activeTab === 'chart' ? 'bg-emerald-500/15 text-emerald-500' : 'text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-tertiary)]'}`}>
                <Target className="h-4 w-4 shrink-0" /> <span>Progress Chart</span>
              </button>
            </div>
          </div>
            ...
            <div className="p-3 sm:p-5">
              <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-[var(--color-text-tertiary)]">Interactive lesson checks and standalone quizzes for {titleOf(selectedCourse!.title)}.</p>
                </div>
                <label className="flex min-h-11 w-full items-center gap-2 rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-tertiary)] px-3 py-2 sm:w-auto">
                  ...
                  <select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as 'all' | ActivityType)} className="w-full bg-transparent text-xs font-bold text-[var(--color-text-primary)] outline-none sm:w-auto">
              ...
              <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                          <div className="min-w-0">
                            <h3 className="break-words text-sm font-black text-[var(--color-text-primary)]">{row.title}</h3>
                            <p className="mt-1 text-[11px] leading-4 text-[var(--color-text-tertiary)]">{row.chapterTitle || 'General'} • {formatDate(row.date)}</p>
                          </div>
                          <span className={`w-fit rounded-full px-2.5 py-1 text-[11px] font-bold ${row.type === 'quiz' ? 'bg-blue-500/15 text-blue-400' : 'bg-emerald-500/15 text-emerald-400'}`}>
                ...
                {filteredActivities.length === 0 && (
                  <div className="rounded-xl border border-dashed border-[var(--color-border-default)] px-4 py-12 text-center">
                    <ListChecks className="mx-auto h-8 w-8 text-[var(--color-text-tertiary)]" />
                    <p className="mt-3 text-sm font-black text-[var(--color-text-secondary)]">{courseActivities.length === 0 ? 'No activity yet for this course' : 'No activity matches this filter'}</p>
                    <p className="mx-auto mt-1 max-w-md text-xs leading-5 text-[var(--color-text-tertiary)]">{courseActivities.length === 0 ? 'Complete an interactive lesson check or quiz and it will appear here.' : 'Choose All Activity or another activity type to see your results.'}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {activeTab === 'chart' && (
            <div className="space-y-5 p-3 sm:p-5">
              ...
              {chartRows.length > 0 ? (
                ...
                <div className="rounded-xl border border-dashed border-[var(--color-border-default)] px-4 py-14 text-center text-sm text-[var(--color-text-tertiary)]">Complete a lesson check or quiz in this course to see the progress chart.</div>
              )}

              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-4 sm:p-5">
                  <div className="mb-5 flex items-center gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-400"><TrendingUp className="h-5 w-5" /></span>
                ...
                <div className="rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-4 sm:p-5">
                  <div className="mb-5 flex items-center gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-500/15 text-red-400"><TrendingDown className="h-5 w-5" /></span>
