    ...
    <div className="p-6 lg:p-10 pt-20 lg:pt-10">
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard icon="📚" label="Published Courses" value={stats!.courses.published} color="bg-green-500" />
          ...
          <StatCard icon="📝" label="Total Courses" value={stats!.courses.total} color="bg-amber-500" />
        ...
        <div className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-6 shadow-card">
          <h2 className="text-lg font-bold text-[var(--color-text-primary)] mb-4">📈 Enrollment Overview</h2>
        ...
        {/* Course Distribution */}
        <div className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-6 shadow-card">
          <h2 className="text-lg font-bold text-[var(--color-text-primary)] mb-4">📂 Course Distribution</h2>
          <div className="space-y-3">
            {stats!.courseDistribution.map((c) => (
              <div key={c.category} className="flex items-center gap-3">
                <span className="w-20 text-sm font-medium text-[var(--color-text-secondary)]">{catLabels[c.category] || c.category}</span>
                ...
                <span className="w-8 text-sm font-semibold text-[var(--color-text-primary)] text-right">{c.count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-6 shadow-card">
          <h2 className="text-lg font-bold text-[var(--color-text-primary)] mb-4">📅 Monthly Registrations</h2>
            ...
            {stats!.monthlyRegistrations.map((m) => {
              ...
              return (
                <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-xs font-semibold text-[var(--color-text-primary)]">{m.count}</span>
                  <div className="w-full rounded-t-lg bg-primary-500 transition-all duration-500" style={{ height: `${height}%`, minHeight: m.count > 0 ? '8px' : '2px' }} />
