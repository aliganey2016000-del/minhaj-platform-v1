...
function StudentPicker({ value, onChange }: { value: Student | null; onChange: (s: Student | null) => void }) {
  ...
  if (value) return (
    <div className="flex items-center justify-between gap-3 rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-secondary)] px-4 py-3">
      <div className="min-w-0"><p className="truncate text-sm font-semibold text-[var(--color-text-primary)]">{studentName(value)}</p><p className="font-mono text-xs text-[var(--color-text-tertiary)]">{value.studentId}</p></div>
      <button type="button" onClick={() => onChange(null)} className="rounded-lg p-1.5 text-[var(--color-text-tertiary)] hover:bg-[var(--color-surface-tertiary)]"><X className="h-4 w-4" /></button>
    </div>
  );

  return (
    <div ref={ref} className="relative">
      <div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--color-text-tertiary)]" /><input value={query} onChange={e => { setQuery(e.target.value); setOpen(true); }} onFocus={() => setOpen(true)} placeholder="Search by student ID, phone, or name..." className="w-full rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] py-3 pl-9 pr-4 text-sm outline-none focus:border-primary-500" /></div>
      {open && query.trim().length >= 2 && <div className="absolute z-30 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] shadow-2xl">
        {loading ? <p className="px-4 py-4 text-center text-xs text-[var(--color-text-tertiary)]">Searching...</p> : results.length === 0 ? <p className="px-4 py-4 text-center text-xs text-[var(--color-text-tertiary)]">No students found.</p> : results.map(s => <button key={s._id} type="button" onClick={() => { onChange(s); setQuery(''); setOpen(false); }} className="flex w-full items-center justify-between gap-3 border-b border-[var(--color-border-subtle)] px-4 py-3 text-left last:border-0 hover:bg-[var(--color-surface-secondary)]"><span><span className="block text-sm font-semibold text-[var(--color-text-primary)]">{studentName(s)}</span><span className="block font-mono text-xs text-[var(--color-text-tertiary)]">{s.studentId}</span></span><span className="text-xs text-[var(--color-text-tertiary)]">Select</span></button>)}</div>}
    </div>
  );
}
...
export function PaymentsRecordInvoice() {
  ...
  useEffect(() => {
    ...
    api.get(`/invoices/student/${student._id}`)
      .then(({ data }) => {
        const all = (data.data || []) as Invoice[];
        const open = all.filter(inv => inv.status !== 'void' && Number(inv.amountDue) > 0);
        setInvoices(open);
        const first = open[0];
        setSelectedId(first?._id || '');
        setAmount(first ? String(first.amountDue) : '');
      })
      .catch(err => { setInvoices([]); setSelectedId(''); setAmount(''); setError(err.response?.data?.message || 'Failed to load student invoices'); })
  ...
  const chooseInvoice = (id: string) => {
    setSelectedId(id);
    const next = invoices.find(i => i._id === id);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setMessage(`${money(amountNumber)} payment recorded against ${invoice.title}. Remaining invoice balance: ${money(Math.max(0, invoice.amountDue - amountNumber))}.`);
      setReview(false);
    ...
    <div className="min-h-screen bg-[var(--color-surface-secondary)] p-4 pt-20 sm:p-6 lg:p-10 lg:pt-10">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.35fr_.65fr]">
          ...
          <aside className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-5 shadow-card sm:p-6"><p className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-tertiary)]">Payment rule</p><h2 className="mt-1 text-lg font-bold">Invoice-first collection</h2><div className="mt-5 space-y-3 text-sm text-[var(--color-text-secondary)]"><p>1. Select the student.</p><p>2. Select an existing outstanding invoice.</p><p>3. Payment amount is checked against that invoice's remaining balance.</p><p>4. Payment is posted with the invoice ID.</p><p>5. No invoice means no payment can be recorded.</p></div></aside>
        </div>
      </div>

      {review && student && invoice && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={() => !loading && setReview(false)}><div className="w-full max-w-md rounded-2xl bg-[var(--color-surface-primary)] p-6 shadow-2xl" onClick={e => e.stopPropagation()}><h2 className="text-lg font-bold">Confirm Payment</h2><div className="mt-4 space-y-3 rounded-xl bg-[var(--color-surface-secondary)] p-4 text-sm"><div className="flex justify-between gap-3"><span>Student</span><strong>{studentName(student)}</strong></div><div className="flex justify-between gap-3"><span>Invoice</span><strong className="text-right">{invoice.title}</strong></div><div className="flex justify-between"><span>Invoice balance before</span><strong>{money(invoice.amountDue)}</strong></div><div className="flex justify-between"><span>Payment</span><strong className="text-primary-600">{money(amountNumber)}</strong></div><div className="flex justify-between"><span>Balance after</span><strong className="text-emerald-600">{money(Math.max(0, invoice.amountDue - amountNumber))}</strong></div><div className="flex justify-between"><span>Method</span><strong>{method.replace('_', ' ')}</strong></div></div><div className="mt-5 flex gap-2"><button type="button" onClick={() => setReview(false)} disabled={loading} className="flex-1 rounded-xl border border-[var(--color-border-default)] px-4 py-2.5 text-sm font-semibold">Back</button><button type="button" onClick={submit} disabled={loading} className="flex-1 rounded-xl bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{loading ? 'Recording...' : 'Confirm & Record'}</button></div></div></div>}
    </div>
  );
}
