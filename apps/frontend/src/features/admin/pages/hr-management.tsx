...
const cards = [
  ...
  { title: 'Institution Structure', description: 'Manage departments and, for universities, faculties.', to: '/admin/hr?tab=structure', icon: Building2 },
];
...
function InstitutionStructure() {
  ...
  useEffect(() => {
    if (!isUniversity) setActiveTab('department');
  }, [isUniversity]);
  ...
  const load = async () => {
    ...
    try {
      const [departmentResponse, facultyResponse] = await Promise.all([
        ...
        isUniversity ? api.get(`/departments/faculties${schoolQuery}`) : Promise.resolve({ data: { data: [] } }),
      ]);
  ...
  return <div className="space-y-6 p-4 sm:p-6">
    <div className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-6 shadow-sm">
      ...
      <h1 className="mt-1 text-2xl font-bold">{isUniversity ? 'Faculty & Department Management' : 'Department Management'}</h1>
      <p className="mt-1 text-sm text-[var(--color-text-secondary)]">{isUniversity ? 'University hierarchy: Institution → Faculty → Department.' : 'This institution uses Institution → Department. Faculties are not used.'}</p>
    </div>
    ...
    {(!isUniversity || activeTab === 'faculty') && isUniversity && <section className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-5 shadow-sm">
    ...
    {(!isUniversity || activeTab === 'department') && <section className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-5 shadow-sm">
      <div className="flex items-center justify-between"><div><h2 className="font-semibold">Departments</h2><p className="text-xs text-[var(--color-text-tertiary)]">{isUniversity ? 'Every department must belong to a faculty.' : 'Departments belong directly to the institution.'}</p></div><UsersRound className="h-5 w-5 text-[var(--color-text-tertiary)]" /></div>
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        ...
        {isUniversity && <select className={`${inputClass} sm:col-span-2`} value={departmentFacultyId} onChange={e => setDepartmentFacultyId(e.target.value)}><option value="">Select faculty...</option>{faculties.map(f => <option key={f._id} value={f._id}>{f.name}{f.code ? ` (${f.code})` : ''}</option>)}</select>}
      ...
      <div className="mt-4 divide-y rounded-xl border">{departments.map(d => <div key={d._id} className="flex items-center justify-between gap-3 px-3 py-3"><div><p className="text-sm font-medium">{d.name}</p><p className="text-xs text-[var(--color-text-tertiary)]">{d.code || 'No code'}{isUniversity && d.facultyId ? ` · ${d.facultyId.name}` : ''}</p></div><div className="flex gap-1"><button className="rounded-lg p-2 hover:bg-[var(--color-surface-tertiary)]" onClick={() => editDepartment(d)} title="Edit"><Pencil className="h-4 w-4" /></button><button className="rounded-lg p-2 text-red-600 hover:bg-red-50" onClick={() => removeDepartment(d._id)} title="Delete"><Trash2 className="h-4 w-4" /></button></div></div>)}{!loading && departments.length === 0 && <p className="px-3 py-6 text-center text-sm text-[var(--color-text-tertiary)]">No departments yet.</p>}</div>
    </section>}
  </div>;
}
