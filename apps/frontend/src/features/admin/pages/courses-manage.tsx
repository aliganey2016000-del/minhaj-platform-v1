...
function CourseModal({ course, org, onClose, onSaved }: { course?: Course; org: Org; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({ title: course?.title?.en || '', courseCode: course?.courseCode || '', duration: course?.duration || 8, fee: course?.fee || 0, maxStudents: course?.maxStudents || 50, teacher: course?.teacher?._id || '', classId: course?.class?._id || '', status: course?.status || 'draft' as Status, description: course?.description?.en || '' });
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}><div className="w-full max-w-xl max-h-[92vh] overflow-y-auto rounded-2xl bg-[var(--color-surface-primary)] p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
    ...
    <form onSubmit={submit} className="space-y-3">
      ...
      <label className="block text-xs font-semibold">Course Code<input maxLength={50} value={form.courseCode} onChange={e => setForm({ ...form, courseCode: e.target.value })} placeholder="e.g. MATH-101" className="mt-1 w-full rounded-xl border border-[var(--color-border-default)] bg-transparent px-3 py-2 text-sm" /></label>
...
export default function CoursesManage() {
  ...
  const bulkUpdateStatus = async (nextStatus: 'published' | 'draft') => {
    if (!selected.length || !window.confirm(`${nextStatus === 'published' ? 'Publish' : 'Move to draft'} ${selected.length} selected course(s)?`)) return;
    setBulkStatusSaving(true); setError('');
    try {
      await api.patch('/courses/bulk/status', { ids: selected, status: nextStatus });
      setCourses(current => current.map(course => selected.includes(course._id) ? { ...course, status: nextStatus } : course));
      setSelected([]);
    } catch (err: any) { setError(err.response?.data?.message || 'Bulk status update failed'); }
  ...
  return <div className="space-y-4 p-4 sm:p-6">
    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h1 className="truncate text-xl font-bold">Manage Courses</h1><p className="text-sm text-[var(--color-text-secondary)]">{courses.length} courses · {labelFor(type)} curriculum</p></div><div className="relative shrink-0"><button aria-label="Course actions" onClick={() => setMenuOpen(v => !v)} className="rounded-xl border border-[var(--color-border-default)] p-2.5 hover:bg-[var(--color-surface-tertiary)]"><MoreVertical className="h-5 w-5" /></button>{menuOpen && <div className="absolute right-0 top-11 z-30 w-56 rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-1.5 shadow-xl"><button onClick={() => { setModal('new'); setMenuOpen(false); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-[var(--color-surface-tertiary)]"><Plus className="h-4 w-4" /> Add Course</button><button onClick={() => { exportCourses(); setMenuOpen(false); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-[var(--color-surface-tertiary)]"><Download className="h-4 w-4" /> Export Courses</button><button onClick={() => { setShowImportModal(true); setMenuOpen(false); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-[var(--color-surface-tertiary)]"><Upload className="h-4 w-4" /> Import Courses</button>{selected.length > 0 && <><div className="my-1 border-t border-[var(--color-border-subtle)]" /><button disabled={bulkStatusSaving} onClick={() => { setMenuOpen(false); void bulkUpdateStatus('published'); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-emerald-600 hover:bg-emerald-50 disabled:opacity-50 dark:hover:bg-emerald-950/20"><Send className="h-4 w-4" /> Publish Selected ({selected.length})</button><button disabled={bulkStatusSaving} onClick={() => { setMenuOpen(false); void bulkUpdateStatus('draft'); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-amber-600 hover:bg-amber-50 disabled:opacity-50 dark:hover:bg-amber-950/20"><FilePenLine className="h-4 w-4" /> Move Selected to Draft</button><button disabled={bulkStatusSaving} onClick={() => { bulkDelete(); setMenuOpen(false); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-red-600 hover:bg-red-50 disabled:opacity-50"><Trash2 className="h-4 w-4" /> Delete Selected ({selected.length})</button></>}</div>}</div></div>
