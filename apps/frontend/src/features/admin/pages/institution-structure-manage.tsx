...
export function InstitutionStructureManage() {
  ...
  useEffect(() => { if (!usesFaculty && tab === 'faculty') setTab(showPrograms ? 'program' : 'department'); }, [usesFaculty, showPrograms, tab]);
  ...
  const saveFaculty = async () => {
    ...
    try {
      ...
      setFacultyForm(emptyFaculty); setFacultyEdit(null); setMessage(facultyEdit ? 'Faculty updated.' : 'Faculty created.'); await load();
  ...
  const saveDepartment = async () => {
    ...
    try {
      ...
      setDepartmentForm(emptyDepartment); setDepartmentEdit(null); setMessage(departmentEdit ? 'Department updated.' : 'Department created.'); await load();
  ...
  const saveProgram = async () => {
    ...
    try {
      ...
      setProgramForm(emptyProgram); setProgramEdit(null); setMessage(programEdit ? 'Program updated.' : 'Program created.'); await load();
  ...
  return <div className="space-y-6 p-4 sm:p-6">
    ...
    {tab === 'faculty' && usesFaculty && <section className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-5">
    ...
    {tab === 'program' && showPrograms && <section className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-5">
