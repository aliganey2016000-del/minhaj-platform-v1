...
import { Building2, Download, MoreVertical, Pencil, Plus, Trash2, Upload } from 'lucide-react';
...
type Department = { _id: string; name: string; code?: string; numberOfStudents?: number; headOfDepartment?: string; phone?: string; email?: string; establishedYear?: number; facultyId?: { _id: string; name: string } | null };
...
export function InstitutionStructureManage() {
  ...
  const [exportingDepartments, setExportingDepartments] = useState(false);
  ...
  const exportDepartments = async () => {
    setExportingDepartments(true); setError(''); setDepartmentActionsOpen(false);
    try {
      const response = await api.get('/departments/export', { params: { school: organizationId }, responseType: 'blob' });
      const url = URL.createObjectURL(response.data);
      const link = document.createElement('a');
      link.href = url;
      link.download = `departments-export-${new Date().toISOString().slice(0, 10)}.xlsx`;
      document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url);
      setMessage('Departments exported successfully.');
    } catch (err: any) { setError(err.response?.data?.message || 'Failed to export departments.'); }
    finally { setExportingDepartments(false); }
  };
  ...
  return <div className="space-y-6 p-4 sm:p-6">
    ...
    {tab === 'department' && <section className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-5">
      <div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">Departments</h2><p className="text-xs text-[var(--color-text-tertiary)]">{usesFaculty ? 'Every department must belong to a faculty.' : 'Departments belong directly to this institution.'}</p></div><div className="relative"><button type="button" aria-label="Department actions" aria-haspopup="menu" aria-expanded={departmentActionsOpen} onClick={() => setDepartmentActionsOpen(v => !v)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] transition hover:bg-[var(--color-surface-tertiary)]"><MoreVertical className="h-5 w-5" /></button>{departmentActionsOpen && <><button type="button" aria-label="Close department actions" onClick={() => setDepartmentActionsOpen(false)} className="fixed inset-0 z-30 cursor-default" /><div role="menu" className="absolute right-0 top-12 z-40 w-56 overflow-hidden rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-1.5 shadow-xl"><button type="button" role="menuitem" onClick={() => { setDepartmentActionsOpen(false); setDepartmentEdit('new'); }} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium hover:bg-[var(--color-surface-tertiary)]"><Plus className="h-4 w-4 text-primary-600" />Add Department</button><button type="button" role="menuitem" onClick={() => { setDepartmentActionsOpen(false); setShowDepartmentImport(true); }} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium hover:bg-[var(--color-surface-tertiary)]"><Upload className="h-4 w-4 text-primary-600" />Import Departments</button><button type="button" role="menuitem" disabled={exportingDepartments} onClick={exportDepartments} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium hover:bg-[var(--color-surface-tertiary)] disabled:opacity-50"><Download className="h-4 w-4 text-primary-600" />{exportingDepartments ? 'Exporting...' : 'Export Departments'}</button></div></>}</div></div>
      ...
      <div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b"><th className="px-3 py-3">Code</th><th className="px-3 py-3">Department</th><th className="px-3 py-3">Number of Students</th><th className="px-3 py-3 text-right">Actions</th></tr></thead><tbody>{departments.map(d => <tr key={d._id} className="border-b"><td className="px-3 py-3">{d.code || '—'}</td><td className="px-3 py-3 font-medium">{d.name}</td><td className="px-3 py-3">{d.numberOfStudents || 0}</td><td className="relative px-3 py-3 text-right"><button type="button" aria-label={`Actions for ${d.name}`} aria-expanded={departmentActionId === d._id} onClick={() => setDepartmentActionId(current => current === d._id ? null : d._id)} className="rounded-lg p-2 hover:bg-[var(--color-surface-tertiary)]"><MoreVertical className="h-4 w-4" /></button>{departmentActionId === d._id && <div className="absolute right-3 top-11 z-20 w-36 rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-1 shadow-xl"><button type="button" onClick={() => { setDepartmentActionId(null); setDepartmentEdit(d._id); setDepartmentForm({ name: d.name, code: d.code || '', headOfDepartment: d.headOfDepartment || '', phone: d.phone || '', email: d.email || '', establishedYear: d.establishedYear ? String(d.establishedYear) : '', facultyId: d.facultyId?._id || '' }); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-[var(--color-surface-tertiary)]"><Pencil className="h-4 w-4" />Edit</button><button type="button" onClick={() => { setDepartmentActionId(null); void remove('department', d._id); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" />Delete</button></div>}</td></tr>)}</tbody></table>{!loading && departments.length === 0 && <p className="py-8 text-center text-sm text-[var(--color-text-tertiary)]">No departments yet.</p>}</div>
    </section>}
    ...
    {showDepartmentImport && <BulkEntityImportModal title="Import Departments" description="Download the official template, upload an Excel/CSV file, or paste rows directly. Existing departments with the same name are updated safely." templateUrl={`/departments/template?school=${encodeURIComponent(organizationId)}`} importUrl={`/departments/import?school=${encodeURIComponent(organizationId)}`} templateName="departments-template.xlsx" headers={['Department Name', 'Code', 'Faculty', 'Head of Department', 'Phone', 'Email', 'Established Year']} onClose={() => setShowDepartmentImport(false)} onImported={load} />}
  </div>;
}
