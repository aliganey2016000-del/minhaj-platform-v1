...
function RowActionsMenu({ onEdit, onRemove, removeLabel }: { onEdit: () => void; onRemove?: () => void; removeLabel?: string }) {
  ...
  return (<>
    ...
    {open && btnRef.current && createPortal(
      <div ref={menuRef} style={{ position: 'fixed', top: btnRef.current.getBoundingClientRect().bottom + 4, right: window.innerWidth - btnRef.current.getBoundingClientRect().right, zIndex: 100 }} className="w-44 rounded-xl border border-slate-100 dark:border-slate-800 bg-[var(--color-surface-primary)] shadow-elevated py-1">
        ...
        {onRemove && <button onClick={() => { setOpen(false); onRemove(); }} className="flex w-full items-center gap-2 px-3.5 py-2.5 text-xs font-medium text-red-600 hover:bg-[var(--color-surface-tertiary)] transition-colors"><Ban className="h-3.5 w-3.5" strokeWidth={1.75} /> {removeLabel || 'Deactivate'}</button>}
      </div>, document.body,
    )}
  </>);
}
...
export function UsersManage() {
  ...
  // A user whose Student/Teacher/Parent creation failed partway through
  // leaves an orphaned User+Profile behind (see repair-orphaned-student-user.ts) —
  // invisible on Manage Students/Teachers/Parents, but still listed here.
  // Deactivating one is useless (it can never be reactivated into a real
  // account), so the backend removes it outright once it's already inactive
  // and has no linked domain record; otherwise this still just deactivates.
  // Refetch rather than patch local state so either outcome renders correctly.
  const handleRemove = async (u: UserRecord) => {
    const confirmText = u.isActive
      ? 'Deactivate this user? They will no longer be able to log in.'
      : 'Delete this inactive account? If it has no linked Student/Teacher/Parent record it will be removed permanently and the email freed up for reuse.';
