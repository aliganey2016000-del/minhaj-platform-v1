...
export interface LessonItem { _id: string; title: string; type: 'lesson'; content?: string; videoUrl?: string; videoDuration?: number; featuredImage?: string; attachments: Attachment[]; order: number; status: 'draft' | 'published'; duration: number; deliveryMode: LessonDeliveryMode; contentBlocks?: ContentBlock[]; defaultMinReadSeconds?: number; blockForwardSeeking?: boolean; videoCheckpoints?: VideoCheckpoint[]; createdAt?: string; updatedAt?: string; _isNew?: boolean; _isEditing?: boolean; }
export interface BaseQuizQuestion { _id?: string; question: string; explanation?: string; points?: number; }
...
export type QuizQuestion = McqQuestion | TrueFalseQuestion | MatchingQuestion | OrderingQuestion | PictureChoiceQuestion | SwipeSortQuestion | ListenWriteQuestion | FillBlankQuestion | WordScrambleQuestion | SentenceBuildQuestion | ShortAnswerQuestion;
export function normalizeQuestion(q: any): QuizQuestion { if (!q || !q.type) return { ...q, type: 'mcq', options: q?.options?.length >= 2 ? q.options : ['', ''], correctIndex: q?.correctIndex ?? 0 }; const base = { ...q }; switch (q.type) { case 'mcq': return { ...base, options: q.options ?? ['', ''], correctIndex: q.correctIndex ?? 0 } as QuizQuestion; case 'true_false': return { ...base, correctAnswer: q.correctAnswer ?? true } as QuizQuestion; case 'matching': return { ...base, pairs: q.pairs ?? [] } as QuizQuestion; case 'ordering': return { ...base, items: q.items ?? [] } as QuizQuestion; case 'picture_choice': return { ...base, choices: q.choices ?? [], correctIndex: q.correctIndex ?? 0 } as QuizQuestion; case 'swipe_sort': return { ...base, cards: q.cards ?? [], leftLabel: q.leftLabel ?? 'Left', rightLabel: q.rightLabel ?? 'Right' } as QuizQuestion; case 'listen_write': return { ...base, audioUrl: q.audioUrl ?? '', hint: q.hint ?? '' } as QuizQuestion; case 'fill_blank': return { ...base, textTemplate: q.textTemplate ?? '', blanks: q.blanks ?? [], distractors: q.distractors ?? [] } as QuizQuestion; case 'word_scramble': return { ...base, answer: q.answer ?? '', hint: q.hint ?? '' } as QuizQuestion; case 'sentence_build': return { ...base, words: q.words ?? [], distractors: q.distractors ?? [] } as QuizQuestion; case 'short_answer': return { ...base, correctAnswers: Array.isArray(q.correctAnswers) ? q.correctAnswers : [] } as QuizQuestion; default: return base as QuizQuestion; } }
export interface QuizItem { _id: string; title: string; type: 'quiz'; description?: string; questions: QuizQuestion[]; passingScore: number; timeLimit?: number; order: number; status: 'draft' | 'published'; duration: number; createdAt?: string; updatedAt?: string; _isNew?: boolean; _isEditing?: boolean; }
export interface AssignmentItem { _id: string; title: string; type: 'assignment'; description?: string; instructions?: string; dueDate?: string; maxScore: number; allowedFileTypes?: string[]; attachments: Attachment[]; order: number; status: 'draft' | 'published'; duration: number; createdAt?: string; updatedAt?: string; _isNew?: boolean; _isEditing?: boolean; }
export interface ExamItem { _id: string; title: string; type: 'exam'; examId: string; examDate?: string; totalMarks?: number; order: number; status: 'draft' | 'published'; duration: number; createdAt?: string; updatedAt?: string; _isNew?: boolean; }
