...
function StudentModal({ student, schools, onClose, onSaved }: {
  ...
  useEffect(() => {
    ...
    (async () => { try { const { data } = await api.get('/classes', { params: { schoolId: form.school, status: 'active', limit: '200' } }); setClasses(data.data || []); } catch { setClasses([]); } })();
  }, [form.school, isEdit]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[var(--color-surface-primary)] rounded-2xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="text-xs font-semibold text-[var(--color-text-primary)] mb-1 block">Student ID</label>
            <input className={`${ic('studentId')} font-mono uppercase`} name="studentId" placeholder="Leave blank to auto-generate" value={form.studentId} onChange={handleChange} />
            {errors.studentId && <p className="mt-1 text-xs text-red-500">{errors.studentId}</p>}
            <p className="mt-1 text-[11px] text-[var(--color-text-tertiary)]">Optional — leave blank for an auto-generated ID (e.g. STU-2026-0001), or enter your own. Only checked for duplicates within this organization.</p>
          </div>
...
function ApproveModal({ student, schools, onClose, onDone }: { student: Student; schools: SchoolBrief[]; onClose: () => void; onDone: () => void }) {
  ...
  useEffect(() => { if (!school) { setClasses([]); setClassId(''); return; } (async () => { try { const { data } = await api.get('/classes', { params: { schoolId: school, status: 'active', limit: '200' } }); setClasses(data.data || []); } catch { setClasses([]); } })(); }, [school]);
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="w-full max-w-2xl rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] shadow-2xl">
              ...
              <div className="px-6 py-5 space-y-6">
                ...
                {importMode === 'paste' && (<div className="space-y-3"><div className="rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-secondary)] p-4"><p className="text-xs font-semibold text-[var(--color-text-secondary)] mb-2">Paste your spreadsheet data below, including the header row (tab-separated columns — this is exactly what you get selecting a range in Excel and copying it):</p><p className="text-xs text-[var(--color-text-tertiary)] mb-3 font-mono">Student ID &nbsp; First Name &nbsp; Last Name &nbsp; Gender &nbsp; Email &nbsp; Password &nbsp; Organization &nbsp; Class Name &nbsp; Section &nbsp; Grade &nbsp; Enrollment Date &nbsp; Medical Notes &nbsp; Guardian Name &nbsp; Guardian Email &nbsp; Guardian Password &nbsp; Guardian Phone &nbsp; Relationship <span className="italic">(Student ID, Password, Grade, Medical Notes and Guardian Password are optional; Class Name + Section must match an existing class exactly; Organization only needed if you manage more than one)</span></p><textarea value={pasteText} onChange={(e) => { setPasteText(e.target.value); setPasteError(''); }} rows={8} placeholder={"Paste data from Excel here, including the header row...\n\nExample:\nStudent ID\tFirst Name\tLast Name\tGender\tEmail\tPassword\tOrganization\tClass Name\tSection\tGrade\tEnrollment Date\tMedical Notes\tGuardian Name\tGuardian Email\tGuardian Password\tGuardian Phone\tRelationship\n\tAhmed\tAli\tmale\tahmed@example.com\t\tMadrasa Al-Noor\tGrade 3\tA\t\t2026-01-15\t\tMohamed Ali\tparent@example.com\t\t+252612345678\tFather"} className="w-full rounded-lg border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] px-3 py-2.5 text-xs font-mono text-[var(--color-text-primary)] placeholder-[var(--color-text-tertiary)] focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 resize-y" /></div>{pasteError && <div className="rounded-lg border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-950/20 px-3 py-2 text-xs text-red-600 dark:text-red-400">{pasteError}</div>}{parsedRows.length > 0 && (<div className="rounded-xl border border-[var(--color-border-default)] overflow-hidden"><div className="bg-[var(--color-surface-secondary)] px-4 py-2 text-xs font-semibold text-[var(--color-text-tertiary)]">Preview — {parsedRows.length} row{parsedRows.length !== 1 ? 's' : ''} parsed</div><div className="max-h-40 overflow-auto"><table className="w-full text-xs"><tbody className="divide-y divide-[var(--color-border-subtle)]">{parsedRows.slice(0, 20).map((row, ri) => (<tr key={ri} className={ri % 2 === 0 ? 'bg-[var(--color-surface-primary)]' : 'bg-[var(--color-surface-secondary)]'}>{row.map((cell, ci) => (<td key={ci} className="px-3 py-1.5 text-[var(--color-text-secondary)] whitespace-nowrap border-r border-[var(--color-border-subtle)] last:border-r-0">{cell}</td>))}</tr>))}</tbody></table></div></div>)}</div>)}
