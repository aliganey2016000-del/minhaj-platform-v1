...
function StatusPill({ status }: { status?: Status | null }) {
  ...
  const cls = status === 'present' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700';
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
        ...
        <div className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-4 shadow-card">
          <p className="font-bold text-[var(--color-text-primary)]">Reception Check-in / Check-out</p><p className="mb-3 text-xs text-[var(--color-text-tertiary)]">Use Student ID. A successful check-in marks the student Present.</p>
      ...
      {data && data.roster.length > 0 && <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{[['Students', data.summary.total], ['Present', data.summary.present], ['Absent', data.summary.absent], ['Unmarked', data.summary.unmarked]].map(([label, value]) => <div key={String(label)} className="rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-3 text-center"><p className="text-xs text-[var(--color-text-tertiary)]">{label}</p><p className="text-xl font-bold text-[var(--color-text-primary)]">{value}</p></div>)}</div>}
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] shadow-card">
        ...
        {loading ? <div className="p-10 text-center text-sm text-[var(--color-text-tertiary)]">Loading daily roster...</div> : visible.length === 0 ? <div className="p-10 text-center text-sm text-[var(--color-text-tertiary)]">No students found for this date.</div> : <div className="divide-y divide-[var(--color-border-subtle)]">{visible.map((row) => {
          ...
          return <div key={row._id} className="p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center"><div className="min-w-0 flex-1"><p className="font-semibold text-[var(--color-text-primary)]">{row.name}</p><p className="text-xs text-[var(--color-text-tertiary)]">{row.studentId} · {className(row)}</p><div className="mt-1 flex items-center gap-2"><StatusPill status={row.effectiveStatus}/>{row.effectiveStatus === 'absent' && (row.daily?.reasonCode || draft.reasonCode) && <span className="rounded-full bg-blue-100 px-2 py-1 text-[10px] font-semibold text-blue-700">Excused</span>}{!row.daily && row.derivedStatus && <span className="text-[10px] text-[var(--color-text-tertiary)]">from period attendance</span>}</div></div><div className="grid grid-cols-2 gap-2 sm:flex"><button type="button" onClick={() => setDrafts((current) => ({ ...current, [row._id]: { ...draft, status: 'present', reasonCode: '', arrivalTime: '', departureTime: '' } }))} className={`rounded-lg border px-4 py-2.5 text-xs font-bold ${draft.status === 'present' ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-[var(--color-border-default)] text-[var(--color-text-secondary)]'}`}>Present</button><button type="button" onClick={() => setDrafts((current) => ({ ...current, [row._id]: { ...draft, status: 'absent' } }))} className={`rounded-lg border px-4 py-2.5 text-xs font-bold ${draft.status === 'absent' ? 'border-red-600 bg-red-600 text-white' : 'border-[var(--color-border-default)] text-[var(--color-text-secondary)]'}`}>Absent</button></div></div>{draft.status === 'absent' && <div className="mt-3 grid gap-2 sm:grid-cols-[auto_minmax(180px,1fr)_minmax(180px,1fr)] sm:items-center"><label className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-[var(--color-border-default)] px-3 text-xs font-semibold text-[var(--color-text-secondary)]"><input type="checkbox" checked={!!draft.reasonCode} onChange={(e) => setDrafts((current) => ({ ...current, [row._id]: { ...draft, reasonCode: e.target.checked ? (draft.reasonCode || 'other') : '' } }))}/>Excused</label>{draft.reasonCode && <select value={draft.reasonCode} onChange={(e) => setDrafts((current) => ({ ...current, [row._id]: { ...draft, reasonCode: e.target.value as Reason } }))} className="rounded-lg border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] px-2 py-2.5 text-xs">{REASONS.filter((reason) => reason.value).map((reason) => <option key={reason.value} value={reason.value}>{reason.label}</option>)}</select>}<input value={draft.notes} onChange={(e) => setDrafts((current) => ({ ...current, [row._id]: { ...draft, notes: e.target.value } }))} placeholder="Note (optional)" className="rounded-lg border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] px-2 py-2.5 text-xs"/></div>}</div>;
        })}</div>}
