/**
 * Content Blocks Import Modal — bulk-creates Interactive Gate Content
 * Blocks (each with its own Stop & Check question(s)) from an uploaded
 * Excel file or pasted spreadsheet rows. Mirrors CourseContentImportModal's
 * UI, but unlike that one this never touches the database directly — it
 * only parses the file and hands the resulting blocks back to the Lesson
 * Editor, which appends them to its local contentBlocks state exactly like
 * "+ Add Content Block" and "AI Generate" already do (saved later via the
 * normal Save Lesson / Save All Changes flow).
 */
...
// when it puts data on the clipboard as plain text, so a naive split('\n')
// would shred a multi-line Block Content cell into bogus extra rows.
...
interface ContentBlocksImportModalProps {
  onClose: () => void;
  defaultMinReadSeconds: number;
  onImported: (blocks: ContentBlock[]) => void;
}
...
export function ContentBlocksImportModal({ onClose, defaultMinReadSeconds, onImported }: ContentBlocksImportModalProps) {
  ...
  const [result, setResult] = useState<{ blocksImported: number; questionsImported: number; errors: { row: number; message: string }[] } | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const handleDownloadTemplate = async () => {
    try {
      const token = localStorage.getItem('accessToken') || '';
      const response = await fetch(`${api.defaults.baseURL}/content-blocks-import/template`, {
      ...
      link.href = url;
      link.download = 'content-blocks-template.xlsx';
  ...
  const applyParsed = (parsed: ParseResult) => {
    const blocks: ContentBlock[] = parsed.blocks.map((b, i) => ({
      _id: generateTempId(),
      order: i,
      title: b.title,
      content: b.content,
      minReadSeconds: b.minReadSeconds || defaultMinReadSeconds,
      questions: b.questions.map((q): ContentBlockQuestion =>
        q.type === 'mcq'
          ? {
              type: 'mcq',
              question: q.question,
    ...
    const questionsImported = blocks.reduce((sum, b) => sum + (b.questions?.length || 0), 0);
    if (blocks.length > 0) {
      onImported(blocks);
    }
    if (blocks.length > 0 && parsed.errors.length === 0) {
      onClose();
    } else {
      setResult({ blocksImported: blocks.length, questionsImported, errors: parsed.errors });
    }
  };
  ...
  const submitFileImport = async () => {
    ...
    try {
      ...
      const { data } = await api.post('/content-blocks-import/parse', formData);
  ...
  const submitPasteImport = async () => {
    ...
    const file = new File([blob], 'pasted-content-blocks.csv', { type: 'text/csv' });
    setImporting(true);
    setError('');
    setResult(null);
    setPasteError('');
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="border-b border-[var(--color-border-subtle)] px-6 py-5">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-xl font-bold text-[var(--color-text-primary)]">Import Content Blocks</h2>
              <p className="text-sm text-[var(--color-text-tertiary)] mt-1">Bulk-create blocks and their Stop &amp; Check questions from a spreadsheet.</p>
            </div>
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button type="button" onClick={() => { setMode('paste'); setPasteError(''); }} className={`rounded-xl border-2 p-4 text-left transition-all ${mode === 'paste' ? 'border-primary-500 bg-primary-50 dark:bg-primary-950/20 shadow-sm' : 'border-[var(--color-border-default)] hover:border-[var(--color-border-strong)] bg-[var(--color-surface-primary)]'}`}>
              <span className="text-2xl block mb-1">📋</span>
        ...
        {result && (
          <div className="border-t border-[var(--color-border-subtle)] px-6 py-4 space-y-2">
            <p className="text-sm font-semibold text-[var(--color-text-primary)]">
              {result.blocksImported} block{result.blocksImported === 1 ? '' : 's'} added, {result.questionsImported} question{result.questionsImported === 1 ? '' : 's'}
...
export default ContentBlocksImportModal;
